...
async function fetchFreshData(endpoint, cacheKey, retries = 2) {
    // In the browser, route through server proxy to avoid exposing API key
    // and to leverage server-side caching, rate limiting, and deduplication
    if (typeof window !== "undefined") {
        ...
        for (let attempt = 0; attempt <= retries; attempt++) {
            try {
                ...
                // 429 — honour server Retry-After header
        ...
        // All retries exhausted — try returning stale cache before giving up
        const stale = tmdbCache.get(cacheKey);
        if (stale) {
            console.warn("TMDB proxy failed, serving stale cache for:", endpoint);
            return stale.value;
        }

        // Return safe empty result instead of crashing the page
        console.error("TMDB proxy fetch failed after retries:", lastError?.message || lastError);
        return { results: [] };
    }
...
export const tmdb = {
    ...
    getMovieDetails: async (id) => {
        const data = await fetchTMDB(
            `movie/${id}?append_to_response=credits,similar,videos,release_dates`
        );
        // If TMDB returned empty/error response, return null so pages can handle gracefully
    ...
    getTVDetails: async (id) => {
        ...
        const data = await fetchTMDB(
            `tv/${id}?append_to_response=credits,similar,videos,content_ratings`
        );
    ...
    getStronglyRelated: async (id, type, currentItem) => {
        try {
            // Fetch potential candidates from multiple sources (3 API calls)
            ...
            // Combine all candidates, dedupe, remove self
            const allCandidates = [
                ...similar.results,
            ...
            // Pre-score candidates using data already in list results
            // (genre_ids, vote_average, release_date) — no extra API calls
            const currentGenreIds = currentItem.genres?.map((g) => g.id) || [];
            const currentYear = currentItem.release_date
                ? new Date(currentItem.release_date).getFullYear()
                : currentItem.first_air_date
                    ? new Date(currentItem.first_air_date).getFullYear()
                    : null;

            const preScored = uniqueCandidates.map((candidate) => {
                let preScore = 0;
                // Genre overlap (~40% weight) — list results have genre_ids array
                const candidateGenreIds = candidate.genre_ids || [];
                if (currentGenreIds.length > 0 && candidateGenreIds.length > 0) {
                    const overlap = currentGenreIds.filter((gid) => candidateGenreIds.includes(gid)).length;
                    const maxGenres = Math.max(currentGenreIds.length, candidateGenreIds.length);
                    preScore += (overlap / maxGenres) * 40;
                }
                // Rating proximity (5%)
                if (currentItem.vote_average && candidate.vote_average) {
                    const ratingDiff = Math.abs(currentItem.vote_average - candidate.vote_average);
                    preScore += Math.max(0, (10 - ratingDiff) / 10) * 5;
                }
                // Year proximity (5%)
                const candYear = candidate.release_date
                    ? new Date(candidate.release_date).getFullYear()
                    : candidate.first_air_date
                        ? new Date(candidate.first_air_date).getFullYear()
                        : null;
                if (currentYear && candYear) {
                    const yearDiff = Math.abs(currentYear - candYear);
                    preScore += Math.max(0, (20 - yearDiff) / 20) * 5;
                }
                return { ...candidate, preScore };
            });

            // Sort by pre-score, take top 10 for detailed scoring (was 30)
            preScored.sort((a, b) => b.preScore - a.preScore);
            const topCandidates = preScored.slice(0, 10);

            // Full scoring with credits/keywords — only 10 API calls instead of 30
            ...
            // Fallback: if too few results, include top-scored candidates
    ...
    getWatchProviderRegions: async () => {
        try {
            const data = await fetchTMDB(`watch/providers/regions`);
            return data;
        } catch (error) {
    ...
    /**
     * Get movie release dates (for theatrical info)
     */
    getMovieReleaseDates: async (id) => {
        try {
