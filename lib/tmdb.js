...
async function fetchFreshData(endpoint, cacheKey, retries = 2) {
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
        ...
        // If we reach here, all retries failed. Fallback to mock data.
        console.warn(`TMDB API failed for ${endpoint}. Using mock data.`);
        const mock = getMockData(endpoint);
        // Cache the mock data so we don't keep retrying and failing
        tmdbCache.set(cacheKey, mock);
        return mock;
    }
...
export const tmdb = {
    ...
    getStronglyRelated: async (id, type, currentItem) => {
        try {
            ...
            // Filter by threshold (60%) and sort by score
            const threshold = 60;
