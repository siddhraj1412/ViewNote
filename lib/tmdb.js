...
function getProxyUrl(endpoint) {
    return `/api/tmdb?endpoint=${encodeURIComponent(endpoint)}`;
}
...
async function fetchFreshData(endpoint, cacheKey, retries = 2) {
    ...
    const url = isBrowser()
        ...
        : `${TMDB_BASE_URL}/${endpoint}${endpoint.includes("?") ? "&" : "?"}api_key=${TMDB_API_KEY}`;
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(
                    `TMDB API Error: ${response.status} ${response.statusText}${errorBody ? ` — ${errorBody}` : ""}`
                );
            }
...
export const tmdb = {
    ...
    getTVSeasonDetails: async (tvId, seasonNumber) => {
        const data = await fetchTMDB(
            `tv/${tvId}/season/${seasonNumber}?append_to_response=credits,images`
        );
        return data;
    },
    ...
    getTVEpisodeDetails: async (tvId, seasonNumber, episodeNumber) => {
        const data = await fetchTMDB(
            `tv/${tvId}/season/${seasonNumber}/episode/${episodeNumber}?append_to_response=credits,images`
        );
        return data;
    },
    ...
    getStronglyRelated: async (id, type, currentItem) => {
        try {
            ...
            // The previous scoring approach fetched per-candidate details which can trigger
            // many TMDB requests and lead to 429s / slow page loads.

            const [similar, recommendations, byGenre] = await Promise.all([
                fetchTMDB(`${type}/${id}/similar`).catch(() => ({ results: [] })),
                fetchTMDB(`${type}/${id}/recommendations`).catch(() => ({ results: [] })),
            ...
            const unique = Array.from(
                new Map(allCandidates.filter(Boolean).map((item) => [item.id, item])).values()
