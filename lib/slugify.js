 ...
 * @param {string} [type] - "movie" or "tv" or "episode". Falls back to item.mediaType or item.media_type.
...
export function getMediaUrl(item, type) {
    ...
    if (mediaType === 'episode') return getEpisodeUrl(item);
    return '/';
}

/**
 * Generate URL for a TV episode.
 * Routes to the show page with season/episode info.
 *
 * @param {object} episode - Episode object with seriesId, series_name, seasonNumber, episodeNumber
 * @returns {string} URL path e.g. "/show/breaking-bad-1396/season/1/episode/1"
 */
export function getEpisodeUrl(episode) {
    if (!episode) return '/';
    const seriesId = episode.seriesId || episode.showId || episode.series_id;
    const seriesName = episode.series_name || episode.seriesName || episode.showName || episode.title || '';
    if (!seriesId) return '/';
    const slug = generateSlug(seriesName);
    const base = slug ? `/show/${slug}-${seriesId}` : `/show/${seriesId}`;
    if (episode.seasonNumber != null && episode.episodeNumber != null) {
        return `${base}/season/${episode.seasonNumber}/episode/${episode.episodeNumber}`;
    }
    return base;
}
