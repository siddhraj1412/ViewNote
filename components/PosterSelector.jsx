...
export default function PosterSelector({ isOpen, onClose, mediaId, mediaType, defaultPoster }) {
    ...
    const loadUserCustomization = async () => {
        ...
        if (customization?.customPoster) {
            setSelectedPoster(customization.customPoster);
        }
    };
    ...
    const fetchPosters = async () => {
        try {
            ...
            showToast.error("Failed to load posters");
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleSave = async () => {
        ...
        const result = await customizationService.updatePoster(
            user.uid,
            mediaId,
            mediaType,
            selectedPoster
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Select Poster" maxWidth="1200px">
            {/* Content */}
            <div className="p-6">
                {loading ? (
            ...
            <div className="flex items-center justify-between p-6 border-t border-white/10 bg-secondary/50 backdrop-blur-sm sticky bottom-0">
                ...
                <div className="flex gap-3">
                    ...
                    <button
                        onClick={handleSave}
                        disabled={saving || selectedPoster === defaultPoster}
