...
import { Calendar, Heart, Tv } from "lucide-react";
...
export default function ShowsDiaryYearPage() {
    ...
    useEffect(() => {
        ...
        const fetchDiary = async () => {
            setLoading(true);
            try {
                // Fetch all ratings for this user with mediaType=tv
                ...
                console.error("Error fetching show diary:", err);
            ...
            <div className="site-container py-10">
                <div className="flex items-center justify-between gap-4 mb-8">
                    <div className="min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                            ...
                            <h1 className="text-3xl md:text-4xl font-bold truncate">Shows Diary - {year}</h1>
                        </div>
                ...
                {loading ? (
                    ...
                    <div className="bg-secondary rounded-xl border border-white/5 p-6">
                        <div className="text-sm text-textSecondary">No show diary entries for {year}.</div>
                    </div>
                ) : (
                    <div className="space-y-2">
                        {displayItems.map((item) => (
                            <Link key={item.id} href={getMediaUrl({ id: item.mediaId, title: item.title, name: item.title }, "tv")} className="block">
