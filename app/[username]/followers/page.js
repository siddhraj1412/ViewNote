...
export default function FollowersPage() {
    ...
    const loadFirst = useCallback(async (userId) => {
        ...
        setLoading(true);
        try {
            const result = await followService.getFollowers(userId, PAGE_SIZE, null);
    ...
    const loadMore = useCallback(async () => {
        ...
        setLoadingMore(true);
        try {
            const result = await followService.getFollowers(uid, PAGE_SIZE, lastDoc);
            ...
            <div className="site-container py-10">
                <div className="flex items-center justify-between gap-4 mb-8">
                    <div className="min-w-0">
                        <h1 className="text-3xl md:text-4xl font-bold truncate">Followers</h1>
                ...
                {loading ? (
                    <div className="text-center py-12 text-textSecondary">Loading followers...</div>
                ) : users.length === 0 ? (
                    <div className="bg-secondary rounded-xl border border-white/5 p-6">
                        <div className="text-sm text-textSecondary">No followers yet.</div>
                    </div>
                ) : (
